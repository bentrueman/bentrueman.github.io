...
`pbcusol` can be used to generate lead and copper solubility predictions that are comparable with those found in literature *CITE SCHOCK*, as detailed in the package README on [GitHub](https://github.com/bentrueman/pbcusol). (They're reproduced here using the `wesanderson` package for colour palettes [@wes].)
...
list(
  ...
  scale_colour_gradientn(colours = palette, labels = function(breaks) 1e-3 * (10 ^ breaks)) +
  labs(
