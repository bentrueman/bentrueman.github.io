...
Recently, I've been experimenting with the excellent R package `tidyphreeqc` [@tidyphreeqc], which provides a convenient interface for generating PHREEQC input files [@phreeqc_r; @phreeqc]. `tidyphreeqc` incorporates easily into my workflow, and it's found its way into several recent publications *CITE LI*.
...
`pbcusol` can be used to generate lead and copper solubility predictions that are comparable with those found in literature *CITE SCHOCK*, as detailed in the package README on [GitHub](https://github.com/bentrueman/pbcusol).

```{r basic-grid, cache=TRUE}
...
nom_plot <- nom_grid %>% 
  ...
  geom_raster() + 
  geom_contour(aes(z = pb_ppb), col = "black") +
  labs(
    x = expression("Humic/fulvic acid (mg C L"^-1*")"),
    y = "pH",
...
nom_plot %>% 
  ...
  patchwork::plot_annotation(
    title = "Impact of residual humic/fulvic acid on lead solubility",
    subtitle = expression("Solubility is controlled by hydrocerussite (Pb"[3]*"(CO"[3]*")"[2]*"(OH)"[2]*")"),
    caption = expression("pH 7.5, 5 mg C L"^-1)
  )
```
