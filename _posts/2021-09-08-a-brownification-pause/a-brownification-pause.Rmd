  ...
  mutate(
    ...
    model = map(data_df,
      ~ carx(
        ...
        CI.compute = FALSE,
        p = 1, 
        seed = 123#3412#56432# 842
      )
    ),
    preds = map2(model, data_df,
      ~ predict(.x, newxreg = .y, n.ahead = nrow(.y))
    ),
    out = map2(model, preds,
      ~ tibble(
        fit = fitted(.x),
        pred = .y$fit,
        lwr = .y$ci[,1],
...
model %>% 
  ...
  # geom_line(aes(col = fct_recode(name, "Model" = "pred", "Data" = "value"))) +
  geom_point(
    data = function(x) x %>%
      filter(
        # name == "value", 
  ...
  labs(x = NULL, y = "Apparent colour (Pt-Co)", col = NULL, shape = NULL)
```
The residuals showed no clear deviations from whiteness; here, the grey band denotes $\pm1.96/\sqrt{n}$, the approximate 95% confidence bands on the autocorrelation function of a white noise series.
...
model <- model %>% 
  mutate(
    ...
    resid_carx = map(model, residuals),
