...
But overall, there's not much evidence here that the GAM describes this particular series any better than the linear model. And a GAM fit using `brms` ("Bayesian Regression Models using Stan")---which accomodates both left-censoring and autoregression---yielded similar results. Here is a sketch of the model one might fit using `brms::brm()`. Basic diagnostic plots suggest that this model needs work, but it is worth noting that it yields a smooth curve that differs little from the censored autogregression with linear time covariate. Fitted values from the latter model---including the AR(1) component---are superimposed in red on the following plot.
...
preds %>%
  ...
  geom_line(
    ...
    aes(y = med_pred, linetype = sem, col = "brms")
  ) +
  geom_point(
    data = function(x) x %>%
