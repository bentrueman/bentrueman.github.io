...
Then, define a couple of functions that we will use to build the model. The first is a wrapper around `pbcusol::eq_sol_fixed()` that saves having to code the same arguments repeatedly. The second builds a list of the arguments to `pbcusol::eq_sol_fixed()` that change over the iterations.
