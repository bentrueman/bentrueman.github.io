...
Recently, I've encountered several regression problems where the predictors are partially censored. To tackle this issue, I turned to Paul B&uuml;rkner's excellent package for Bayesian regression, `brms` [@brms]. Paul's has written a helpful [vignette](https://cran.r-project.org/web/packages/brms/vignettes/brms_missings.html) on handling missing values. The general strategy we'll use here is outlined in that vignette: imputation during model fitting by declaring the missing values as parameters. That is, we'll fit a multivariate model that imputes missing values in both the predictor and the response variable. The only problem is that `brms` doesn't handle censored predictors, so we'll need to customize our approach a little bit.
