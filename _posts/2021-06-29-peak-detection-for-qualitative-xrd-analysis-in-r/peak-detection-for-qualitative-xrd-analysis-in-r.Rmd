...
stds <- read_csv("xrd_standards.csv") %>% 
  filter(phase %in% c("Cerussite", "Hydrocerussite")) %>% 
  unit_scale() # scale to [0,1]
```

Preamble

The data represent a mixed lead carbonate phase containing cerussite (PbCO~3~) and hydrocerussite (Pb~3~(CO~3~)~2~(OH)~2~).
